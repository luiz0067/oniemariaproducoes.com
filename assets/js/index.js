...
function processa_resposta_servidor(data){
	...
	try{	
		if (data!==undefined){
			...
			else if(data.mensagem_erro!=undefined){
				$(".mensagem_erro").html(data.mensagem_erro);
				$(".mensagem_erro").removeClass("d-none");
			}
			else if(data.mensagem_informacao!=undefined){
			...
			for(posicao_registro in data.registros){
				...
				for(var posicao_valor in registro){
					...
					if(tipos[posicao_valor]=="boolean"){
						...
						_img=$('<img>');
						_img.attr('src','../uploads/album/160x120/'+_valor);
						_link=$('<a>');
						_link.attr('href','../uploads/album/1024x768/'+_valor);
			...
			$(".miniexcluir").click(function(event) {
				...
				form_atual.find(':input').not(':button, :submit, :reset, :checkbox, :radio').val('');
	...
	$.ajax(
		{
			...
			xhr: function() {
				...
				xhr.upload.addEventListener("progress", function(evt) {
					if (evt.lengthComputable) {
						...
						if(percentItem>=100){
							files_item.shift()
							$(".progress-item").find(".progress-bar").html("processando informações");
						...
						if(percentComplete>=100){
							$(".progress-all").find(".progress-bar").html("processando informações");
            ...
            error: function(xhr,er) {
				...
				$(".progress-all").removeClass("bg-success");				
                $('#loader').hide();
				$(".mensagem_erro").html("Não foi possivel Salvar houve um erro.");
				$(".mensagem_erro").removeClass("d-none");
            }
		}
	);
